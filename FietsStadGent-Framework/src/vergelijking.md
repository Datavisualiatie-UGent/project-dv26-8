...
const selectLocations = Inputs.checkbox(locations.map(d => d.name), {
  value: [locations[0].name, locations[1].name]
});
...
<div class="page">
  ...
  <section class="card card--detail">
    ...
    <article class="card card--detail">
      ...
      ${resize((width) => drukte(normalizedMonthlyPerLocation.filter(d => selectedLocations.includes(d.location)), "Aantal fietsers", {width, height: 400}))}
    </article>
  </section>
